...
// #region Tools
var DEFAULT_SPLITER = '@dq@'

function getDecodeName (field) {
    var decodeName = field.name.split(DEFAULT_SPLITER)[0]
    if (field.agg) {
        decodeName = field.agg + '(' + decodeName + ')'
    }
    return decodeName
}
    ...
    switch (formatType) {
        ...
        case FieldFormatTypes.Currency:
            var _a = config,
                ...
                unit = _a.unit,
                useThousandSeparator = _a.useThousandSeparator;
        ...
        case FieldFormatTypes.Percentage:
            ...
            if (isNaN(formattedValue)) {
                ...
                formattedValue = formartByDecimalPlaces(formattedValue, config.decimalPlaces) + "%";
            }
            break;
        ...
        case FieldFormatTypes.Date:
            formattedValue = global.moment(value).format(config.format)
...
function findFieldByExpression (fields, expr) {
    if (!fields || !fields.length) { return null }
    if (!expr) { return null }
    ...
    while (true) {
        ...
        var fieldName = getDecodeName(fields[i])
        if (fieldName === expr) {
            field = fields[i]
            break
        }
...
function getFormattedDataRow(fields, dataRow) {
    var obj = {}
    for (var currentDataKey in dataRow) {
        var formattedValue = dataRow[currentDataKey]
        var field = findFieldByExpression(fields, currentDataKey)
        if (field) {
            if (field.format) {
                formattedValue = getFormattedValue(formattedValue, field.format)
            }
        }
        obj[currentDataKey] = formattedValue
    }
    return obj
}

function getWidgetFields (widgetConfig) {
    var withNoAggregators = false
    var chartStyles = widgetConfig.chartStyles
    if (chartStyles.table) {
        withNoAggregators = chartStyles.table.withNoAggregators
    }

    var cols = widgetConfig.cols
    var rows = widgetConfig.rows
    var metrics = []
    for (var i = 0; i < widgetConfig.metrics.length; i++) {
        var m = widgetConfig.metrics[i]
        if (withNoAggregators) {
            m.agg = ''
        }
        metrics.push(m)
    }
    var fields = cols.concat(rows).concat(metrics)
    return fields
}

function getFormattedDataRows(widgetConfigJson, dataRows) {
    if (!widgetConfigJson) {
        return dataRows
    }
...
// #region FieldConfig Parser

function extractQueryVars(expression, withBoundaryToken) {
    var queryVars = []
    if (!expression) {
        return queryVars
    }
    var varReg = /\$(\w+)\$/g
    expression.replace(varReg, function(match, p) {
        var queryVar = p
        if (withBoundaryToken) {
            queryVar = match
        }
        var queryVar = p
        if (withBoundaryToken) {
        ...
        if (queryVars.indexOf(queryVar) < 0) {
...
function getFieldAlias(field, queryVars) {
    if (!field) { return '' }

    var fieldConfig = field.field
    if (!fieldConfig) { return getDecodeName(field) }

    if (!queryVars) {
        queryVars = {}
    }
    var alias = fieldConfig.alias
    var useExpression = fieldConfig.useExpression
    if (!useExpression) {
        return alias
    }

    var queryKeys = extractQueryVars(alias, true)
    var keys = []
    var vals = []
    ...
    for (var i = 0; i < queryKeys.length; i++) {
        ...
        keys.push(queryKey)
        var queryVarVal = queryVars[queryKey]
    ...
    var funcBody = alias
    if (alias.indexOf('return') < 0) {
        funcBody = 'return ' + funcBody
    }
    var paramNames = ['Moment'].concat(keys).concat([funcBody])
    try {
        var func = Function.apply(null, paramNames)
        var params = [Moment].concat(vals)
        var dynamicAlias = func.apply(null, params)
        return dynamicAlias
    } catch (e) {
        return ''
    }
...
function rowsToUse(headerConfig) {
    var children = headerConfig.children
    var childrenRows = headerConfig.isGroup && children.length ? 1 : 0
    if (headerConfig.isGroup) {
        for (var i = 0; i < children.length; i++) {
            childrenRows = lcm(childrenRows, rowsToUse(children[i]))
        }
    }
    return 1 + childrenRows
}

function colsToUse(cfg) {
    if (!cfg.isGroup || !cfg.children.length) {
        return 1
    }
    var colspan = 0
    for (var i = 0; i < cfg.children.length; i++) {
        colspan += colsToUse(cfg.children[i])
    }
    return colspan
}
...
function parseTableHeader(headerConfig, fields, queryVars) {
    ...
    for (var i = 0; i < headerCells.length; i++) {
        ...
        var row = cell.row, col = cell.col, rowspan = cell.rowspan, colspan = cell.colspan
        row = row - 1
        cell.row = row
        cell.range = [row, row + rowspan - 1, col, col + colspan - 1].join(',').split(',')
        if (cell.range[0] === cell.range[1] && cell.range[2] === cell.range[3]) {
            cell.isMerged = false
        } else {
            cell.isMerged = true
        }
        cell.row = cell.row.toString()
        cell.col = cell.col.toString()
        cell.rowspan = cell.rowspan.toString()
        cell.colspan = cell.colspan.toString()
    }
    return headerCells
}
